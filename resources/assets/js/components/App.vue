    ...
    <v-app :if="$auth.ready()">
        ...
        <div class="loading-parent">
            ...
            <v-toolbar app fixed clipped-left v-if="$auth.check() && $user" class="white">
                ...
                <v-toolbar-title class="mx-0 mx-sm-4"><a href="/"><img src="/media/logo.png" alt="logo"
                                                                       class="logo"/></a></v-toolbar-title>
                <SearchBox></SearchBox>
                <v-spacer></v-spacer>
                <v-menu offset-y v-if="$user.UserRole ==='Administratorius'">
                    <v-btn icon slot="activator" class="mx-2" @click="$router.push({name: 'assign'})">
                ...
                <v-menu offset-y>
                    <v-flex shrink v-if="$user && screenWidth > 650" slot="activator" class="hover-bottom-border">
                        <v-icon class="primary--text pr-3">fa-user</v-icon>
                        {{$user.Username}}
                        <v-icon class="pl-2">keyboard_arrow_down</v-icon>
                    </v-flex>
...
<script>
    ...
    import QRCode from './user/modules/QRCode.vue';
    ...
    export default {
        data() {
            return {
                ...
                cartDropdownMeniu: [
                    {
                        text: 'Nauja rezervacija', click: () => {
                            this.$router.push({name: 'cart'})
                        }
                    },
                    {
                        text: 'Įrankių priskyrimas', click: () => {
                            this.$router.push({name: 'assign'})
                        }
                    },
                    {
                        text: 'Įrankių grąžinimas', click: () => {
                ...
                settingsDropdownMeniu: [
                    ...
                    {
                        text: 'Keisti slaptažodį', click: () => {
                            this.$modal.show('change-password-modal')
                        }
                    },
                    {
                        text: 'Atsijungti', click: () => {
